...
ALGOLIA_WEBMOCKS = {
  ...
  # query index
  [:get, %r{^/1/indexes/[^/]+$}] => '{"hits":[{"objectID":42}], "page":0, "hitsPerPage":1}',
  [:post, %r{^/1/indexes/[^/]+/query$}] => '{"hits":[{"objectID":42}], "page":0, "hitsPerPage":1}',
  # delete index
...
module Algolia
  def self.load_webmocks!
    WebMock.stub_request(:any, /.*\.algolia(net\.com|\.net)/).to_return do |request|
