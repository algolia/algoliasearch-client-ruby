  ...
  class GetUserTokenResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Algolia::GetUserTokenResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Algolia::GetUserTokenResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'last_event_at')
    ...
    def list_invalid_properties
      ...
      if @last_event_at.nil?
        invalid_properties.push('invalid value for "last_event_at", last_event_at cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @last_event_at.nil?
    ...
    def hash
      [user_token, last_event_at, scores].hash
    end
