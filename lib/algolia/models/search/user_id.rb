  ...
  class UserId
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Algolia::UserId` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Algolia::UserId`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @nb_records.nil?
        invalid_properties.push('invalid value for "nb_records", nb_records cannot be nil.')
      end

      if @data_size.nil?
        invalid_properties.push('invalid value for "data_size", data_size cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @user_id !~ Regexp.new(/^[a-zA-Z0-9 \-*.]+$/)
      return false if @cluster_name.nil?
      return false if @nb_records.nil?
      return false if @data_size.nil?
      true
    end
    ...
    def hash
      [user_id, cluster_name, nb_records, data_size].hash
    end
