  ...
  module Composition
    class ResultsInjectedItemInfoResponse
      ...
      def self.types_mapping
        {
          :key => :"String",
          :applied_rules => :"Array<ResultsInjectedItemAppliedRulesInfoResponse>"
        }
      end
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::ResultsInjectedItemInfoResponse` initialize method"
          )
        end
