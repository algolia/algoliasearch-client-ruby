...
module Algolia
  module Composition
    class CompositionsSearchResponse
      attr_accessor :run
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::CompositionsSearchResponse` initialize method"
          )
        end

        if attributes.key?(:run)
          if (value = attributes[:run]).is_a?(Array)
            self.run = value
          end
        else
          self.run = nil
