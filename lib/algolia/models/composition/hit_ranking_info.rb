    ...
    class HitRankingInfo
      ...
      def self.types_mapping
        {
          ...
          :composed => :"Hash<String, CompositionIdRankingInfo>"
        }
      end
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::HitRankingInfo` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Algolia::HitRankingInfo`. Please check the name to make sure it's valid. List of attributes: " +
        ...
        if attributes.key?(:composed)
          if (value = attributes[:composed]).is_a?(Hash)
            self.composed = value
          end
        end
      end
