...
module Algolia
  module Composition
    class SearchForFacetValuesParams
      # Search query.
      attr_accessor :query
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::SearchForFacetValuesParams` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Algolia::SearchForFacetValuesParams`. Please check the name to make sure it's valid. List of attributes: " +
      ...
      def hash
        [query, max_facet_hits, search_query].hash
      end
