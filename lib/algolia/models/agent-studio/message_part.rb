      ...
      # List of class defined in oneOf (OpenAPI v3)
        ...
        def build(data)
          ...
          if data.is_a?(Hash) && data.key?("type")
            case data["type"]
            when "reasoning"
              return find_and_cast_into_type(:"ReasoningPart", data)
            when "start"
              return find_and_cast_into_type(:"StartPart", data)
            when "start-step"
              return find_and_cast_into_type(:"StartStepPart", data)
            when "text"
              return find_and_cast_into_type(:"TextPart", data)
            when "tool-approval-request"
              return find_and_cast_into_type(:"ToolApprovalRequestPart", data)
            when "tool-call"
              return find_and_cast_into_type(:"ToolCallPart", data)
            when "tool-result"
              return find_and_cast_into_type(:"ToolResultPart", data)
            end
          end
        ...
        def find_and_cast_into_type(klass, data)
          ...
          when /\AHash<String, (?<sub_type>.+)>\z/
            ...
            const = Algolia::AgentStudio.const_get(klass)
            if const
