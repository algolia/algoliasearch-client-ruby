    ...
    class ToolInvocationV4
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::ToolInvocationV4` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Algolia::ToolInvocationV4`. Please check the name to make sure it's valid. List of attributes: " +
        ...
        if attributes.key?(:args)
          if (value = attributes[:args]).is_a?(Hash)
            self.args = value
          end
        end
        ...
        if attributes.key?(:args_hash)
          self.args_hash = attributes[:args_hash]
        end
      end
