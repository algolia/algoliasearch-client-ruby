    ...
    class SecretKeyResponse
      ...
      def initialize(attributes = {})
        if (!attributes.is_a?(Hash))
          raise(
            ArgumentError,
            "The input argument (attributes) must be a hash in `Algolia::SecretKeyResponse` initialize method"
          )
        end
        ...
        attributes = attributes.each_with_object({}) { |(k, v), h|
          if (!self.class.attribute_map.key?(k.to_sym))
            raise(
              ArgumentError,
              "`#{k}` is not a valid attribute in `Algolia::SecretKeyResponse`. Please check the name to make sure it's valid. List of attributes: " +
      ...
      def hash
        [id, name, value, created_at, updated_at, last_used_at, is_default, agent_ids].hash
      end
