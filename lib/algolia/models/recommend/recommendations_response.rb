  ...
  class RecommendationsResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Algolia::RecommendationsResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Algolia::RecommendationsResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@ab_test_variant_id.nil? && @ab_test_variant_id < 1
        invalid_properties.push('invalid value for "ab_test_variant_id", must be greater than or equal to 1.')
      end

      pattern = Regexp.new(/^(-?\d+(\.\d+)?),\s*(-?\d+(\.\d+)?)$/)
      if !@around_lat_lng.nil? && @around_lat_lng !~ pattern
        invalid_properties.push("invalid value for \"around_lat_lng\", must conform to the pattern #{pattern}.")
      end

      if @hits_per_page.nil?
        invalid_properties.push('invalid value for "hits_per_page", hits_per_page cannot be nil.')
      end
      ...
      if @nb_pages.nil?
        invalid_properties.push('invalid value for "nb_pages", nb_pages cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if !@ab_test_variant_id.nil? && @ab_test_variant_id < 1
      return false if !@around_lat_lng.nil? && @around_lat_lng !~ Regexp.new(/^(-?\d+(\.\d+)?),\s*(-?\d+(\.\d+)?)$/)
      return false if @hits_per_page.nil?
      return false if @hits_per_page > 1000
      return false if @hits_per_page < 1
      return false if @nb_hits.nil?
      return false if @nb_pages.nil?
