...
RSpec.shared_examples 'Reset' do |call_type, timeout|
  context 'when it resets expired hosts' do
    ...
    it 'resets expired hosts according to call_type' do
      ...
      retry_strategy = Algoliasearch::RetryStrategy.new(config)
...
RSpec.describe 'Reset hosts for READ' do
  include_examples 'Reset', READ, 1000
end

RSpec.describe 'Reset hosts for WRITE' do
  include_examples 'Reset', WRITE, 1000
end

RSpec.shared_examples 'Decide' do |call_type, http_response_code, is_timed_out, retry_outcome_code, hosts_length|
  let(:config) { Algoliasearch::SearchConfig.new('app_id', 'api_key') }
...
RSpec.describe 'Decides to retry on 30*/50*' do
...
RSpec.describe 'Decides to retry on timeout' do
...
RSpec.describe 'Decides to fail on 40*' do
...
RSpec.describe 'Decides to succeed on 200' do
